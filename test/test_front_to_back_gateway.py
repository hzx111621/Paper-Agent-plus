...
class FrontToBackFastApiTest(unittest.TestCase):
    ...
    def _client(self):
        ...
        settings = SettingsRepository(
            initial={
                "providers": {"openai": {"api_key": "test-openai-key", "api_base": "https://api.openai.com/v1"}},
    ...
    def _authenticated_api(self, client: TestClient) -> FrontendApiClient:
        """注册一个临时测试账户，并返回已经登录的 API 客户端。"""

        api = self._api(client)
        response = client.post("/api/auth/register", json={"username": "test-user", "password": "test-pass-123"})
        self.assertEqual(response.status_code, 201)
        api.set_auth_token(response.json()["token"])
        return api
    ...
    def test_bootstrap_exposes_local_runtime_capabilities(self):
        ...
        api = self._authenticated_api(client)

        bootstrap = api.bootstrap()
        created = api.create_session("Paper reading")
        sessions = api.list_sessions()

        self.assertFalse(bootstrap["runtime_capabilities"]["multi_chat_socket"])
        self.assertTrue(bootstrap["runtime_capabilities"]["fastapi_rest"])
        self.assertTrue(bootstrap["runtime_capabilities"]["auth_required"])
