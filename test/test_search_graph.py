...
def _analysis_snapshot() -> ProviderSnapshot:
    """返回同时满足子主题分析和全局分析字段要求的固定响应。"""

    # 中文说明：图测试关注节点衔接而不是模型质量，所以用一份同时包含两组字段的 JSON。
    # 每个分析方法只读取自己需要的字段，未带引用的文字会由正式代码补上真实 paperId。
    return _fake_snapshot(
        response_text=(
            '{"研究现状":"已有论文讨论该主题。","一致点":[],"矛盾点":"证据不足。",'
            '"研究空白":"仍需更多研究。","时间线演化":"现有材料较少。",'
            '"技术方法栈演变":"现有材料较少。","领域整体研究概况":"已有论文覆盖该主题。",'
            '"领域全域共性研究共识":"输入材料形成初步共识。",'
            '"领域核心研究争议与矛盾体系":"当前证据不足。",'
            '"领域系统性研究空白与局限":"仍需补充更多证据。",'
            '"领域研究时序演化脉络":"现有论文构成初步时间线。",'
            '"领域技术与研究方法迭代脉络":"现有论文展示了方法变化。",'
            '"各子主题横向差异对比分析":"各子主题覆盖范围不同。",'
            '"领域整体总结与研究展望":"后续可扩大研究范围。"}'
        )
    )


def _outline_snapshot() -> ProviderSnapshot:
    """返回写作节点可以直接执行的一章一节大纲。"""

    return _fake_snapshot(
        response_text=(
            '{"Chapter1":{"title":"主题研究概况","description":"整理已有论文的主要发现。",'
            '"Sections":{"section1":{"title":"主要发现","task":"归纳已有研究的主要发现",'
            '"evidence-map":["领域整体研究概况"],"ref-sections":[],"word-count":200}}}}'
        )
    )


def _writing_snapshot() -> ProviderSnapshot:
    """返回可同时用于正文、审查和摘要步骤的固定响应。"""

    # 中文说明：正文读取 action/content，审查读取 passed，摘要也读取 content，
    # 因此一份稳定 JSON 就能覆盖图测试所需的三个模型调用阶段。
    return _fake_snapshot(
        response_text=(
            '{"action":"draft","content":"现有论文为该研究主题提供了初步证据。",'
            '"paperIds":[],"passed":true,"suggestions":[],"message":"审查通过"}'
        )
    )


def _workflow_overrides(service: "_StubService") -> dict[str, object]:
    """集中提供完整工作流所需的假服务和各阶段假模型。"""

    return {
        "search_node_service": service,
        "search_node_llm": _fake_snapshot(),
        "read_node_llm": _read_snapshot(),
        "analysis_node_llm": _analysis_snapshot(),
        "writing_outline_node_llm": _outline_snapshot(),
        "writing_node_llm": _writing_snapshot(),
    }
...
class GraphTest(unittest.TestCase):
    ...
    def test_run_graph_returns_ranked_papers(self):
        ...
        result = asyncio.run(
            ...
            state_overrides=_workflow_overrides(stub),
            )
        )
