...
class SettingsFastApiTest(unittest.TestCase):
    ...
    def _client(self):
        ...
        repo = SettingsRepository(
            initial={
                "providers": {
                    "openai": {"api_key": "test-openai-key", "api_base": "https://api.openai.com/v1"},
                    "anthropic_compat": {"api_key": "test-anthropic-key", "api_base": "https://proxy.example/v1"},
                },
                "agents": {
                    "default_agent": {
        ...
        app = create_app(settings_repo=repo, sessions_repo=sessions, config=GatewayConfig())
        client = TestClient(app)
        auth = client.post("/api/auth/register", json={"username": "settings-user", "password": "test-pass-123"})
        client.headers.update({"Authorization": f"Bearer {auth.json()['token']}"})
        return repo, client
