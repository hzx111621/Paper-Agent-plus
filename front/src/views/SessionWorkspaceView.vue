...
import { ApiRequestError, cancelSessionRun, createSession, fetchSessionThread, startSessionRun, subscribeSessionRun, type AuthenticatedEventStream } from "../api/sessions";
...
import PaperResultsPanel from "../components/session/PaperResultsPanel.vue";
...
const draft = ref("");
// 中文说明：新会话默认开启精确题目检索，让用户直接粘贴论文标题时能优先找到同名论文。
// 需要做宽泛研究方向检索时，可以在输入框下方的检索设置里关闭它。
const constraints = ref<SessionConstraints>({ search_mode: "paper" });
const threadLoading = ref(false);
const sending = ref(false);
const cancelling = ref(false);
const activeRunId = ref<string | null>(null);
const streamSource = ref<AuthenticatedEventStream | null>(null);
...
async function selectSession(sessionKey: string) {
  ...
  constraints.value = { search_mode: "paper" };
...
/** 为实时流建立带登录令牌的订阅，并把后端发来的每条消息交给前端聚合器整理。 */
...
/** 中文注释：论文卡片重新分析时复用当前会话的实时流，保证进度和普通检索一致。 */
function handlePaperRunStarted(payload: { run_id: string; stream_url: string }) {
  if (!selectedSessionKey.value) return;
  sending.value = true;
  activeRunId.value = payload.run_id;
  openStream(selectedSessionKey.value, payload.stream_url);
}
