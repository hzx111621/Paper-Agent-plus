...
const savingReadDefaults = ref(false);
const savingRetrievalKeys = ref(false);
...
const deepReadLimitDraft = ref("10");
const ieeeXploreKeyDraft = ref("");
const elsevierKeyDraft = ref("");
...
const providerCompletion = computed(() => {
  ...
  const checks = [
    ...
    selectedProvider.value.api_key_required ? Boolean(providerDraft.api_key || providerDraft.api_key_env) : true,
  ];
...
watch(providerBackend, (backend, previousBackend) => {
  if (!isCreatingProvider.value || !backend || backend === previousBackend) {
    return;
  }

  const selectedType = providerTypes.value.find((item) => item.name === backend);
  const previousType = providerTypes.value.find((item) => item.name === previousBackend);
  if (!selectedType) {
    return;
  }

  // 中文说明：新增 Provider 时切换服务类型，只有仍为空或仍是旧默认值的字段才自动替换。
  // 用户自己填写过的地址和环境变量不会被切换操作意外覆盖。
  if (!providerDraft.api_base || providerDraft.api_base === previousType?.default_api_base) {
    providerDraft.api_base = selectedType.default_api_base;
  }
  if (!providerDraft.api_key_env || providerDraft.api_key_env === previousType?.default_api_key_env) {
    providerDraft.api_key_env = selectedType.default_api_key_env;
  }
});
...
watch(settings, (value) => {
  if (value) {
    deepReadLimitDraft.value = String(value.defaults.read.deep_read_limit);
  }
});
...
async function submitReadDefaults() {
  const value = Number(deepReadLimitDraft.value.trim());
  if (!Number.isInteger(value) || value < 0 || value > 200) {
    pushToast({
      tone: "error",
      title: "默认深度阅读数无效",
      description: "请输入 0 到 200 之间的整数。",
    });
    return;
  }

  savingReadDefaults.value = true;
  try {
    settings.value = await saveReadDefaults(value);
    pushToast({
      tone: "success",
      title: "阅读默认值已保存",
      description: `未单独设置时，每次默认深度阅读 ${value} 篇论文。`,
    });
  } catch (error) {
    handleError(error, "保存阅读默认值失败");
  } finally {
    savingReadDefaults.value = false;
  }
}

async function submitPaperRetrievalKeys() {
  if (!ieeeXploreKeyDraft.value.trim() && !elsevierKeyDraft.value.trim()) {
    pushToast({ tone: "error", title: "缺少 API Key", description: "请至少填写 IEEE Xplore 或 Elsevier 的一个 API Key。" });
    return;
  }
  savingRetrievalKeys.value = true;
  try {
    settings.value = await savePaperRetrievalKeys({
      ieee_xplore_api_key: ieeeXploreKeyDraft.value.trim() || undefined,
      elsevier_api_key: elsevierKeyDraft.value.trim() || undefined,
    });
    ieeeXploreKeyDraft.value = "";
    elsevierKeyDraft.value = "";
    pushToast({ tone: "success", title: "论文来源已配置", description: "下一次检索即可使用 IEEE Xplore 和 Elsevier。" });
  } catch (error) {
    handleError(error, "保存论文来源配置失败");
  } finally {
    savingRetrievalKeys.value = false;
  }
}

function preferredProviderType() {
  /** 中文说明：这个项目默认推荐硅基流动，用户没有 OpenAI 或 Anthropic 密钥也能直接完成配置。 */
  return providerTypes.value.find((item) => item.name === "siliconflow") ?? providerTypes.value[0];
}

function primeEditors() {
  if (!creatingProvider.backend) {
    creatingProvider.backend = preferredProviderType()?.name ?? "";
  }
}

function startCreatingProvider() {
  const preferredType = preferredProviderType();
  isCreatingProvider.value = true;
  selectedProviderName.value = "";
  creatingProvider.name = preferredType?.name === "siliconflow" ? "siliconflow_main" : "";
  creatingProvider.backend = preferredType?.name ?? "";
  providerDraft.backend = creatingProvider.backend;
  providerDraft.api_key = "";
  providerDraft.api_key_env = preferredType?.default_api_key_env ?? "";
  providerDraft.api_base = preferredType?.default_api_base ?? "";
...
<template>
  <section class="page-shell settings-page">
    ...
    <template v-else-if="settings">
      ...
      <section class="panel-section default-read-settings">
        <div class="section-heading">
          <div>
            <span class="eyebrow">00 Research Defaults</span>
            <h2>论文阅读默认设置</h2>
            <p>聊天界面不单独填写时，使用这里的深度阅读数量；聊天里的临时设置优先级更高。</p>
          </div>
          <StatusPill tone="neutral" :label="`当前默认 ${deepReadLimitDraft} 篇`" />
        </div>

        <div class="editor-card default-read-settings-editor">
          <label class="field-group">
            <span>默认深度阅读数</span>
            <input
              v-model="deepReadLimitDraft"
              class="field"
              type="number"
              min="0"
              max="200"
              ...
              placeholder="例如 10"
            />
            <small>设置为 0 表示默认不进行全文深度阅读，只保留摘要分析。</small>
          </label>
          <button class="button primary" type="button" :disabled="savingReadDefaults" @click="submitReadDefaults">
            <LoaderCircle v-if="savingReadDefaults" class="spinning" :size="16" />
            <Save v-else :size="16" />
            {{ savingReadDefaults ? "保存中" : "保存阅读默认值" }}
          </button>
        </div>
      </section>
      ...
      <section class="panel-section default-read-settings">
        <div class="section-heading">
          <div>
            <span class="eyebrow">01 Paper Sources</span>
            <h2>论文数据源配置</h2>
            <p>IEEE Xplore 和 Elsevier 需要官方 API Key；密钥只写入本地配置，不会在页面回显。</p>
          </div>
          <div class="heading-meta">
            <StatusPill :tone="settings.paper_retrieval.ieee_xplore_configured ? 'success' : 'warning'" :label="settings.paper_retrieval.ieee_xplore_configured ? 'IEEE 已配置' : 'IEEE 待配置'" />
            <StatusPill :tone="settings.paper_retrieval.elsevier_configured ? 'success' : 'warning'" :label="settings.paper_retrieval.elsevier_configured ? 'Elsevier 已配置' : 'Elsevier 待配置'" />
          </div>
        </div>

        <div class="editor-card default-read-settings-editor">
          <label class="field-group">
            <span>IEEE Xplore API Key</span>
            <input v-model="ieeeXploreKeyDraft" class="field" type="password" autocomplete="new-password" placeholder="已配置时留空即可保留" />
          </label>
          <label class="field-group">
            <span>Elsevier API Key</span>
            <input v-model="elsevierKeyDraft" class="field" type="password" autocomplete="new-password" placeholder="已配置时留空即可保留" />
          </label>
          <button class="button primary" type="button" :disabled="savingRetrievalKeys" @click="submitPaperRetrievalKeys">
            <LoaderCircle v-if="savingRetrievalKeys" class="spinning" :size="16" />
            <Save v-else :size="16" />
            {{ savingRetrievalKeys ? "保存中" : "保存论文来源配置" }}
          </button>
        </div>
      </section>
            ...
            <div class="form-grid provider-form-grid">
              <label v-if="isCreatingProvider" class="field-group">
                ...
                <input
                  ...
                  class="field"
                  type="text"
                  placeholder="例如 siliconflow_main"
                />
              </label>
              ...
              <label class="field-group">
                ...
                <input
                  ...
                  class="field"
                  type="text"
                  :placeholder="selectedProviderType?.default_api_key_env || '例如 SILICONFLOW_API_KEY'"
                />
              </label>
