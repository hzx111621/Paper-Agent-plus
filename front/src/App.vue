...
import { clearAuthToken, getAuthToken, getCurrentUser, logoutAccount } from "./api/auth";
import { createSession, deleteSession, listSessions } from "./api/sessions";
import { archiveSession, renameSession } from "./api/library";
...
/** 中文注释：标题和归档都是轻量管理动作，直接在侧栏完成，避免打断当前研究流程。 */
async function handleRenameSession(sessionKey: string) {
  const session = sessions.value.find((item) => item.key === sessionKey);
  if (!session) return;
  const title = window.prompt("请输入新的会话标题", session.title);
  if (title === null || !title.trim()) return;
  try {
    await renameSession(sessionKey, title.trim());
    await refreshSessions(false);
  } catch (error) { handleError(error, "重命名会话失败"); }
}

async function handleArchiveSession(sessionKey: string) {
  const session = sessions.value.find((item) => item.key === sessionKey);
  if (!session) return;
  try {
    await archiveSession(sessionKey, !session.archived);
    await refreshSessions(false);
    pushToast({ tone: "success", title: session.archived ? "会话已恢复" : "会话已归档", description: "可以在侧栏中继续管理。" });
  } catch (error) { handleError(error, "归档会话失败"); }
}
