...
const manualKeywordsText = computed(() => (props.constraints.keywords ?? []).join(", "));
const sourceOptions: SourceOption[] = [
  { value: "openalex", label: "OpenAlex" },
  { value: "arxiv", label: "arXiv" },
  { value: "semantic_scholar", label: "Semantic Scholar" },
  { value: "ieee_xplore", label: "IEEE Xplore" },
  { value: "elsevier_scopus", label: "Elsevier / Scopus" },
  { value: "google_scholar", label: "Google Scholar", disabled: true, hint: "暂未接入稳定的官方检索接口" },
];
...
/** 切换是否额外按用户输入的原始文字检索同名论文。 */
function setSearchMode(mode: "paper" | "topic") {
  const next = { ...props.constraints, search_mode: mode, exact_title: mode === "paper" };
  emit("update:constraints", next);
}
  ...
  <section class="session-composer-card" :data-variant="props.variant" :aria-busy="running || sending">
    <template v-if="isWelcomeVariant">
      <header class="session-welcome-heading">
        <img class="session-welcome-wordmark" src="/university-wordmark-transparent.png" alt="华东理工大学校徽与校名" />
      ...
      <div class="session-welcome-prompt-card">
        <div class="session-search-mode" role="group" aria-label="检索模式">
          <button type="button" :class="{ active: constraints.search_mode !== 'topic' }" @click="setSearchMode('paper')">搜论文</button>
          <button type="button" :class="{ active: constraints.search_mode === 'topic' }" @click="setSearchMode('topic')">搜主题关键词</button>
        </div>
        ...
        <div v-if="constraintsExpanded" class="session-composer-constraints-panel session-welcome-constraints-panel">
          <div class="session-composer-constraint-grid">
            ...
            <label class="session-composer-constraint-field">
              <span>深度阅读数</span>
              <input class="field" type="number" min="0" max="200" placeholder="系统默认" :value="constraints.deep_read_limit ?? ''" :disabled="running || sending" @input="updateNumberConstraint('deep_read_limit', $event)" />
            </label>
          </div>

          <label class="session-composer-constraint-field session-composer-constraint-wide">
            <span>手动关键词</span>
            <input class="field" type="text" placeholder="留空则由模型生成，用逗号分隔" :value="manualKeywordsText" :disabled="running || sending" @input="updateKeywords" />
          </label>
          ...
          <fieldset class="session-composer-source-field">
            ...
            <label v-for="option in sourceOptions" :key="option.value" class="session-composer-source-option">
              <input type="checkbox" :checked="constraints.sources?.includes(option.value)" :disabled="running || sending || option.disabled" @change="toggleSource(option.value, $event)" />
