...
from src.services.settings import (
    ...
    update_paper_retrieval_keys,
    update_read_defaults,
)
...
def create_settings_router(repo: SettingsRepository, auth_repo: SQLiteSessionRepository) -> APIRouter:
    ...
    @router.get("")
    async def get_settings(request: Request) -> JsonObject:
        """返回完整设置快照。"""

        _require_user(auth_repo, request)
        return settings_payload(repo)

    @router.put("/read-defaults")
    @router.post("/read-defaults")
    async def save_read_defaults(request: Request):
        """保存系统默认深度阅读数量。"""

        try:
            _require_user(auth_repo, request)
            return update_read_defaults(repo, await _json_body(request))
        except SettingsError as exc:
            return _settings_error_response(exc)

    @router.put("/paper-retrieval")
    @router.post("/paper-retrieval")
    async def save_paper_retrieval(request: Request):
        """保存 IEEE Xplore 和 Elsevier 的 API Key。"""

        try:
            _require_user(auth_repo, request)
            return update_paper_retrieval_keys(repo, await _json_body(request))
        except SettingsError as exc:
            return _settings_error_response(exc)

    @router.get("/provider-models", response_model=None)
    async def get_provider_models(request: Request, provider: str = Query(..., min_length=1)):
        """读取某个 provider 的模型目录。"""
...
def _require_user(auth_repo: SQLiteSessionRepository, request: Request) -> None:
    """统一检查设置接口的登录状态。"""

    current_user(auth_repo, request)
