...
def _route_after_search(state: State) -> str:
    """检索没有留下论文时直接整理回复，不再生成没有证据的综述。"""

    # 中文说明：SearchAgent 配置失败、论文来源没有结果或候选论文全部被过滤时，
    # search_results 都会为空。继续执行分析和写作只会产出看似完整但没有文献依据的内容。
    return "run_read" if state.get("search_results") else "compose_reply"


def _route_after_read(state: State) -> str:
    """阅读阶段没有任何结果时直接结束，避免后续节点凭空补写内容。"""

    # 中文说明：正常论文即使只完成摘要阅读也会进入 read_results；列表为空说明
    # 后续分析没有可使用的论文材料，应把已有检索信息交给最终回复节点说明情况。
    return "run_analyse" if state.get("read_results") else "compose_reply"
...
def build_graph():
    ...
    workflow.add_conditional_edges(
        "run_search_agent",
        _route_after_search,
        {"run_read": "run_read", "compose_reply": "compose_reply"},
    )
    workflow.add_conditional_edges(
        "run_read",
        _route_after_read,
        {"run_analyse": "run_analyse", "compose_reply": "compose_reply"},
    )
