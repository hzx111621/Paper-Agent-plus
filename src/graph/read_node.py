...
async def _run_read_node_async(state: State) -> State:
    ...
    deep_read_limit = _deep_read_limit(request.constraints, len(papers), config.deep_read_limit)
...
def _deep_read_limit(constraints: JsonObject, total: int, default_limit: int | None = None) -> int:
    """读取精读数量限制，优先使用聊天设置，否则使用系统默认值。"""

    fallback = total if default_limit is None else default_limit
    raw = constraints.get("deep_read_limit", constraints.get("max_deep_read", fallback))
    try:
