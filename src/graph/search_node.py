...
def run_search_agent_node():
    ...
    async def _node(state: State) -> State:
        ...
        local_papers = _local_papers_from_request(state)
        local_only = bool((state["request"].constraints or {}).get("local_only"))
        # 中文说明：上传的 PDF 即使没有可用模型生成关键词，也应该能单独进入阅读和分析流程。
        if local_papers:
            search_halted = False
        ...
        if search_halted:
            ...
            raw_papers.extend(local_papers)
        ...
        searchable_papers = _filter_searchable_papers(raw_papers, exact_title=intent.topic if intent.exact_title else "")
...
def _local_papers_from_request(state: State) -> list[PaperDocument]:
    """把上传或指定重分析论文转换成统一论文对象。"""

    constraints = state["request"].constraints or {}
    values = constraints.get("local_papers")
    ...
    for item in values:
        ...
        papers.append(
            PaperDocument(
                ...
                venue=str(item.get("source") or "local_upload"),
                source=str(item.get("source") or "local_upload"),
...
async def _execute_search_intent(
    ...
    *,
    ...
    if intent.search_mode == "paper" and intent.topic.strip():
        # 中文说明：模型生成的关键词适合找“相关研究”，但不保证能命中用户输入的完整题目。
        # 打开精确匹配后，再用原始题目单独检索一次，避免题目被模型改写后丢失。
        tasks.append(
            _search_exact_title(
                service,
                intent,
...
async def _search_exact_title(
    service: PaperSearchService,
    intent: SearchIntent,
    *,
    runtime_resources: WorkflowRuntimeResources | None,
) -> tuple[SearchSubtopic, SearchResponse]:
    """用用户输入的原始题目检索，避免模型改写造成同名论文丢失。"""
    ...
    return SearchSubtopic(subtopic="精确论文题目", keyword=intent.topic), response
...
def _filter_searchable_papers(papers: list[PaperDocument], exact_title: str = "") -> list[PaperDocument]:
    """只保留有唯一编号、也有摘要的论文。"""
    ...
    for paper in papers:
        ...
        if not (paper.paperId or "").strip():
            continue
        # 中文说明：精确题目命中的论文即使暂时没有摘要，也要保留在结果列表里，
        # 否则用户明明找到了同名论文，却会在粗筛阶段被悄悄删掉。
        if not (paper.abstract or "").strip() and not _titles_match(paper.title, exact_title):
            continue
        filtered.append(paper)
    return filtered
...
def _score_papers(intent: SearchIntent, papers: list[PaperDocument]) -> list[ScoredPaper]:
    ...
    for paper in papers:
        ...
        exact_title_match = bool(intent.search_mode == "paper" and _titles_match(paper.title, intent.topic))
        ...
        if exact_title_match:
            # 中文说明：完全同名是最强的相关性信号，给足够高的分数确保它排在普通相关论文前面。
            score += 1000.0
        matched_terms = _collect_matched_terms(tokens, phrase_terms, title_text, abstract_text)
        if exact_title_match and intent.topic not in matched_terms:
            matched_terms.insert(0, intent.topic)
        scored_items.append(
            ScoredPaper(
                paper=paper,
                score=score,
...
def _normalize_title(text: str) -> str:
    """统一题目大小写、全半角和标点，减少格式差异造成的误判。"""

    normalized = unicodedata.normalize("NFKC", str(text or "")).lower()
    # 中文注释：题目中常见的冒号、连字符、括号和多余空格不影响是否同名，
    # 这里只保留中文、英文和数字，避免不同来源的标点格式导致匹配失败。
    return "".join(re.findall(r"[\u4e00-\u9fff]|[a-z0-9]", normalized))


def _titles_match(title: str, expected_title: str) -> bool:
    """判断论文标题是否与用户输入的论文题目完全一致。"""

    normalized_title = _normalize_title(title)
    normalized_expected = _normalize_title(expected_title)
    return bool(normalized_title and normalized_expected and normalized_title == normalized_expected)
