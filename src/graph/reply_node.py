...
def run_compose_reply_node():
    ...
    async def _node(state: State) -> State:
        ...
        if final_markdown:
            ...
            # 中文说明：检索规划失败与“正常检索但没有结果”不是同一种情况。
            # 前者优先展示模型配置或返回格式问题，用户才能知道应该去设置页处理什么。
            search_agent_diagnostics = diagnostics.get("agent") if isinstance(diagnostics.get("agent"), dict) else {}
            if summary.get("search_halted"):
                reason = str(search_agent_diagnostics.get("message") or "检索条件没有成功生成").strip()
                assistant_text = f"论文检索未能开始：{reason}"
            else:
                source_errors = dict(summary.get("source_errors") or {})
                if source_errors:
                    assistant_text = "论文来源请求完成，但没有得到可用论文。请检查数据源状态或调整检索条件。"
                else:
                    assistant_text = "未检索到符合条件的论文结果，请尝试放宽年份、排除词或研究主题。"
        else:
