...
class ReadDefaults:
    ...
    # 中文说明：聊天请求没有单独指定数量时，使用这个系统默认值，避免一次处理过多论文。
    ...
    def from_dict(cls, data: Mapping[str, Any] | None) -> "SystemConfig":
        ...
        return cls(
            ...
            paper_retrieval=PaperRetrievalConfig(
                ...
                ieee_xplore_api_key=_optional_text(paper_retrieval.get("ieee_xplore_api_key")),
                elsevier_api_key=_optional_text(paper_retrieval.get("elsevier_api_key")),
            ),
            read=ReadDefaults(
                agent_name=str(read.get("agent_name") or "default_agent"),
                deep_read_limit=_read_non_negative_int(read.get("deep_read_limit"), 10),
