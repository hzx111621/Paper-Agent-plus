...
class OpenAICompatProvider(LLMProvider):
    ...
    def __init__(self, *, spec: ProviderSpec, **kwargs: Any):
        ...
        if self.client is None:
            ...
            self.client = AsyncOpenAI(
                ...
                # 中文说明：项目基类已经统一做重试、计时和 Retry-After 处理，SDK 这里关闭额外重试，
                # 避免一次失败被两层机制重复发送，尤其是硅基流动等兼容网关。
                max_retries=0,
            )
    ...
    def _build_kwargs(
        ...
        if settings.reasoning_effort and self.spec.supports_reasoning_effort:
            # 中文说明：只有明确支持该字段的服务才发送 reasoning_effort；硅基流动等
            # 兼容接口可以通过 extra_body 自己配置思考参数，避免这里发送不认识的字段。
            kwargs["reasoning_effort"] = settings.reasoning_effort
        if stream and self.include_stream_usage and self.spec.supports_stream_options:
            # 中文说明：部分兼容服务不支持 stream_options；注册表关闭该能力时就不发送，
            # 流式正文仍然可以正常返回，只是不强求服务额外返回 token 用量。
