...
class PaperSearchService:
    ...
    def _build_default_connectors(self) -> dict[str, PaperSearchConnector]:
        ...
        ieee_xplore = IeeeXplorePaperConnector(
            api_key=retrieval_config.ieee_xplore_api_key or os.getenv("IEEE_XPLORE_API_KEY"),
        )
        elsevier = ElsevierPaperConnector(
            api_key=retrieval_config.elsevier_api_key or os.getenv("ELSEVIER_API_KEY"),
        )
        return {
            "openalex": openalex,
    ...
    def _select_connectors(
        ...
        for name, connector in self._connectors.items():
            # 中文说明：需要 API Key 的来源没有配置时，不加入“全部来源”检索，
            # 避免每次普通搜索都出现无意义的鉴权错误；用户主动勾选它时仍会得到明确错误提示。
            if not getattr(connector, "configured", True):
                continue
