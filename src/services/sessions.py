...
def list_sessions(repo: SessionRepository, user_id: str | None = None, include_archived: bool = False) -> JsonObject:
    """返回会话列表给前端。"""

    if user_id and hasattr(repo, "list_for_user"):
        return {"sessions": repo.list_for_user(user_id, include_archived=include_archived)}
    return {"sessions": repo.list()}


def fetch_thread(repo: SessionRepository, key: str, user_id: str | None = None) -> JsonObject:
    """返回指定会话的完整线程快照。"""

    if user_id and hasattr(repo, "get_for_user"):
        return repo.get_for_user(key, user_id).thread()
    return repo.get(key).thread()


def create_session(repo: SessionRepository, body: JsonObject | None = None, user_id: str | None = None) -> JsonObject:
    """创建一个新会话。"""

    body = body or {}
    title = str(body.get("title") or "New chat")
    workspace_scope = body.get("workspace_scope")
    if user_id and hasattr(repo, "create_for_user"):
        record = repo.create_for_user(user_id, title=title, workspace_scope=workspace_scope)
    else:
        record = repo.create(title=title, workspace_scope=workspace_scope)
    return {"session": record.summary()}


def delete_session(repo: SessionRepository, key: str, user_id: str | None = None) -> JsonObject:
    """删除一个会话，并返回统一响应。"""

    if user_id and hasattr(repo, "delete_for_user"):
        repo.delete_for_user(key, user_id)
    else:
        repo.delete(key)
