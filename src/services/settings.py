...
def settings_payload(repo: SettingsRepository, agent_name: str | None = None) -> JsonObject:
    ...
    return {
        ...
        "defaults": {
            ...
            "read": {
                "deep_read_limit": system.read.deep_read_limit,
            },
        },
        "paper_retrieval": {
            "ieee_xplore_configured": bool(system.paper_retrieval.ieee_xplore_api_key or os.getenv("IEEE_XPLORE_API_KEY")),
            "elsevier_configured": bool(system.paper_retrieval.elsevier_api_key or os.getenv("ELSEVIER_API_KEY")),
        },
...
def update_read_defaults(repo: SettingsRepository, patch: JsonObject) -> JsonObject:
    """更新系统默认深度阅读数量。"""

    raw_value = patch.get("deep_read_limit", patch.get("deepReadLimit"))
    try:
        value = int(raw_value)
    except (TypeError, ValueError) as exc:
        raise SettingsError("默认深度阅读数必须是整数", 400) from exc
    if value < 0 or value > 200:
        raise SettingsError("默认深度阅读数必须在 0 到 200 之间", 400)
    repo.save_deep_read_limit(value)
    return settings_payload(repo)


def update_paper_retrieval_keys(repo: SettingsRepository, patch: JsonObject) -> JsonObject:
    """保存 IEEE Xplore 和 Elsevier 的检索密钥。"""

    ieee_key = str(patch.get("ieee_xplore_api_key") or "").strip() or None
    elsevier_key = str(patch.get("elsevier_api_key") or "").strip() or None
    if ieee_key is None and elsevier_key is None:
        raise SettingsError("至少填写一个新的论文来源 API Key")
    repo.save_paper_retrieval_keys(
        ieee_xplore_api_key=ieee_key,
...
async def async_provider_models_payload(repo: SettingsRepository, provider: str, client: Any | None = None) -> JsonObject:
    ...
    try:
        ...
        # 中文说明：首次配置时用户通常会先创建 Provider，再同步模型目录，最后才创建 default_agent。
        # 因此模型目录不能因为 default_agent 还没建立就打不开，这里只解析当前步骤真正需要的配置。
        config = ModelConfig(
            providers={
                item_name: _provider_from_dict(item_name, item_value)
                for item_name, item_value in (data.get("providers") or {}).items()
            },
            agents={},
            embedding_profiles=_embedding_profiles_from_dict(data, system),
            system=system,
    ...
    if _provider_requires_key(spec) and not _provider_has_api_key(spec, provider_config):
        return _models_status(
            provider,
            "not_configured",
            "provider api_key 或 api_key_env 尚未提供可用密钥",
        )
    ...
    try:
        if client is not None:
            ...
            catalog_agent = AgentConfig(provider=provider, model_name="__model_catalog__")
            config.agents["__model_catalog__"] = catalog_agent
            # 中文说明：模型工厂要求 default_agent 始终存在。首次配置尚未创建它时，
            # 临时放入同一个目录查询配置；它只存在于内存，不会写回 model.json。
            config.agents.setdefault(config.default_agent, catalog_agent)
...
def _agent_payload(name: str, agent: AgentConfig, provider_config: ProviderConfig) -> JsonObject:
    ...
    return {
        ...
        "has_api_key": _provider_has_api_key(match_provider_backend(provider_config.backend), provider_config),
...
def _provider_configured(spec: ProviderSpec, config: ProviderConfig) -> bool:
    ...
    if not (spec.default_api_base or config.api_base):
        return False
    # 中文说明：运行时本来就支持从环境变量读取密钥，这里的页面状态和保存校验
    # 必须使用同一套判断，否则只填写 api_key_env 时页面会误报“尚未配置”。
    return _provider_has_api_key(spec, config) if _provider_requires_key(spec) else True


def _provider_has_api_key(spec: ProviderSpec, config: ProviderConfig) -> bool:
    """判断配置文件或环境变量中是否存在可用密钥。"""

    if str(config.api_key or "").strip():
        return True
    # 中文说明：用户显式填写的环境变量名优先；没有填写时使用 Provider 自带的默认变量名。
    env_name = str(config.api_key_env or spec.env_key or "").strip()
    return bool(env_name and str(os.getenv(env_name) or "").strip())
...
def _provider_label(name: str) -> str:
    ...
    # 中文说明：内置服务使用官方名称；用户自己创建的 Provider ID 仍按下划线分词展示。
    built_in_labels = {
        "siliconflow": "SiliconFlow（硅基流动）",
        "openai": "OpenAI",
        "openai_compat": "OpenAI 兼容接口",
        "anthropic": "Anthropic",
        "anthropic_compat": "Anthropic 兼容接口",
    }
    return built_in_labels.get(name, name.replace("_", " ").title())
        ...
        items.append(
            {
                ...
                "backend": spec.backend,
                "default_api_base": spec.default_api_base,
                # 中文说明：设置页选择服务类型后可以自动填入建议的环境变量名，
                # 用户无需记住硅基流动等服务分别使用哪个变量。
                "default_api_key_env": spec.env_key,
                "api_key_required": _provider_requires_key(spec),
            }
        )
    return items
