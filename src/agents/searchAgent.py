...
class SearchIntent:
    ...
    search_mode: Literal["paper", "topic"] = "topic"
...
class SearchAgent(BaseAgent):
    ...
    def _run(self, state: JsonObject) -> JsonObject:
        ...
        # 中文说明：搜论文模式不需要模型先拆主题，否则完整题目可能被改写成泛化关键词。
        # 直接保留原始输入，交给搜索节点按论文标题检索。
        if search_mode == "paper":
            plan, raw_model_output, diagnostics = SearchPlan(), None, {
                "used_llm": False,
                "status": "exact_title_mode",
                "message": "已使用原始论文题目进行精确检索。",
            }
        else:
    ...
    def _build_search_intent(self, state: JsonObject, plan: SearchPlan) -> SearchIntent:
        ...
        manual_keywords = self._normalize_string_list(constraints.get("keywords"))
        if manual_keywords:
            plan_keywords = manual_keywords
            plan_subtopics = [SearchSubtopic(subtopic="手动关键词", keyword=" or ".join(manual_keywords))]
        else:
            plan_keywords = list(plan.keywords)
            plan_subtopics = list(plan.subtopics)
        return SearchIntent(
            topic=topic,
            ...
            # 中文说明：检索总量和深度阅读量是两件事，检索仍保留 60 篇候选，
            # 真正进入全文处理的数量由 read 节点的系统默认值或聊天设置决定。
            ...
            # 中文说明：只有用户在检索设置中打开“精确匹配论文题目”时，
            # 搜索节点才会额外拿原始输入去查同名论文，避免普通主题检索被误当成题目检索。
            exact_title=self._search_mode(request) == "paper",
            search_mode=self._search_mode(request),
        )

    def _search_mode(self, request: Any) -> Literal["paper", "topic"]:
        """读取用户选择的检索模式，并兼容之前的 exact_title 设置。"""

        constraints = dict(getattr(request, "constraints", {}) or {})
        raw_mode = str(constraints.get("search_mode") or "").strip().lower()
        if raw_mode == "paper":
            return "paper"
        if raw_mode == "topic":
            return "topic"
        # 中文说明：旧请求没有 search_mode 时，之前的 exact_title=true 仍按论文模式处理，
        # 这样历史会话和已经保存的前端设置不会突然改变含义。
        return "paper" if bool(constraints.get("exact_title", False)) else "topic"
