...
def build_paper_search_tool(service: PaperSearchService | None = None) -> Tool:
    ...
    return Tool(
        spec=ToolSpec(
            name="paper_search",
            description="从 arxiv、OpenAlex、Semantic Scholar、IEEE Xplore 和 Elsevier 等来源检索论文，并返回统一字段结构。",
            parameters_schema={
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "检索关键词或检索短语"},
                    "source": {"type": ["string", "null"], "description": "可选来源名，如 arxiv/openalex/semantic_scholar/ieee_xplore/elsevier_scopus"},
